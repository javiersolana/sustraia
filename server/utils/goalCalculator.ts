 ...
 */
export async function calculateDynamicWeeklyGoal(userId: string, userGoalSetting: number | null): Promise<number> {
    const now = new Date();
    ...
    if (plans.length === 0) {
        return userGoalSetting || 20; // Fallback to user setting or default
    }
    ...
    for (const plan of plans) {
        for (const block of plan.blocks) {
            if (block.distanceMeters) {
                ...
                // Assume avg pace based on block type if not specified
                let paceSecPerKm = 300; // Default 5:00/km

                if (block.type === 'WARMUP' || block.type === 'COOLDOWN') paceSecPerKm = 360; // 6:00/km
                else if (block.type === 'INTERVALS') paceSecPerKm = 240; // 4:00/km
                else if (block.type === 'RUN') paceSecPerKm = 300; // 5:00/km
